...
const updateLocalResolvedState = async (params: {
  owner: string
  repo: string
  pullNumber: number
  userId: string
  ...
  const { prComments } = await adminDb.query({
    prComments: {
      $: {
        where: {
          pullRequestId: pr.id,
          githubId: params.commentId,
        },
        limit: 1,
      },
    },
  })
  const localComment = prComments?.[0]
  if (!localComment) return

  await adminDb.transact(
    adminDb.tx.prComments[localComment.id].update({
...
export const Route = createFileRoute("/api/github/resolve/$owner/$repo/$number")({
  server: {
    handlers: {
      POST: async ({ request, params }) => {
        ...
        try {
          const result = await client.updateReviewComment(owner, repo, parsed.data.commentId, {
            resolved: true,
          })

          await updateLocalResolvedState({
            owner,
            repo,
            pullNumber,
            userId,
          ...
          return jsonResponse({ commentId: result.id, resolved: true })
        } catch (error) {
          ...
          log.error("Unexpected resolve thread error", error, {
            owner,
            repo,
            pullNumber,
            userId,
      ...
      DELETE: async ({ request, params }) => {
        ...
        try {
          ...
          return jsonResponse({ commentId: result.id, resolved: false })
        } catch (error) {
          if (isGitHubAuthError(error)) {
            await handleGitHubAuthError(userId)
            return jsonResponse(
          ...
          log.error("Unexpected unresolve thread error", error, {
            owner,
            repo,
            pullNumber,
            userId,
